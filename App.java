...
<title>Kanban Local — Gestão de Tarefas Simples</title>
<style>
...
<body>
<header><b>Quadro Kanban Local</b> — Organização simples de tarefas</header>
<div class="wrap">

  <h3>Criar tarefa</h3>
  <form id="f">
    <input id="t" placeholder="Título da tarefa" required>
    <textarea id="d" placeholder="Detalhes (opcional)"></textarea>
    <button>Criar</button>
  </form>

  <div class="board">
    <div class="col"><h2>A Fazer</h2><div id="todo"></div></div>
    <div class="col"><h2>Em Andamento</h2><div id="doing"></div></div>
    <div class="col"><h2>Concluídas</h2><div id="done"></div></div>
  </div>
...
function card(t){
  const div = el(`<div class="task">
      ...
      <div class="row">
        <span class="pill">${['A FAZER','EM ANDAMENTO','CONCLUÍDA'][t.status] || t.status}</span>
        <small>criada em: ${new Date(t.criadoEm).toLocaleString()}</small>
      </div>
      <p>${escapeHtml(t.descricao||'')}</p>
      <div class="row">
        ${t.status!==0?'<button data-prev>◀</button>':''}
        ${t.status!==2?'<button data-next>▶</button>':''}
        <button data-del>Remover</button>
      </div>
  </div>`);
